...
/* ----------------------------------------------------------------------
   Contributing authors: Mark Stevens (SNL), Aidan Thompson (SNL)
 				...
 				Manish Gupta, Syracuse University 
------------------------------------------------------------------------- */

#include "fix_nhheat.h"
#include <cstring>
#include <cmath>
#include "atom.h"
#include "force.h"
...
FixNHheat::FixNHheat(LAMMPS *lmp, int narg, char **arg) :
  Fix(lmp, narg, arg),
...

{
	 ...
	 std::cout << narg << " nhheat newarg is printing " <<std::endl;
	
	  //std::cout << newarg[1] << " group " <<std::endl;
	  //std::cout << newarg[2] << " chart temp " <<std::endl;
  if((narg-8)%3 !=0) error->all(FLERR,"Illegal fix nvtheat command");
  numberofregion = (narg-8)/3;
  
	  
  if (narg < 4) error->all(FLERR,"Illegal fix nvtheat command");
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg],"temp") == 0) {
      //if (iarg+4 > narg) error->all(FLERR,"Illegal fix nvtheat command");
      tstat_flag = 1;
      t_start = force->numeric(FLERR,arg[iarg+1]);
      t_target = t_start;
      t_stop = force->numeric(FLERR,arg[iarg+2]);
      t_period = force->numeric(FLERR,arg[iarg+3]);
      std::string fluid = arg[iarg+4];
      if ( fluid == "water")  polarflag = false;
      else if( fluid == "argon") polarflag = true;
      else error->all(FLERR,"fix nvtheat is valid only for argon and water heating over platinum");
      std::cout << numberofregion << " number of region  " <<std::endl;
      for(int j= 1; j<= numberofregion ; j++){
    	  int iregion = domain->find_region(arg[iarg+2+3*j]);
    	  double temperature = force->numeric(FLERR,arg[iarg+3+3*j]);
	      //std::cout <<" strinh " << arg[iarg+3+3*j] <<std::endl;
	      std::string dir = arg[iarg+4+3*j];
	      int direction = 0;
    	  if ( dir == "x")  direction = 0;
    	  else if(dir == "y")   direction = 1;
    	  else if(dir == "z")  direction = 2;
    	  else error->all(FLERR,"Illegal fix nvtheat command");
    	  if (iregion == -1) error->all(FLERR,"Group region ID does not exist");
    	  regionID.push_back(iregion);
    	  temperaturelist.push_back(temperature);
    	  heatdir.push_back(direction);
     	 //std::cout << " temperature  " << temperature <<std::endl;
    	 //std::cout <<" direction  " << dir <<std::endl;
    	  //std::vector<int>* temp = new std::vector<int>;
    	  //CriticalSectionAtomList.push_back(temp);
    	  
    	 // for (int k = 0; k < atom->numberOfRegions; k++) {
    	  //		std::vector<int>* temp = new std::vector<int>;
    	  	//	atom->CriticalSectionAtomList.push_back(temp);
    	  //	}
  	...
  	std::cout << " number of region " << temperaturelist.size() <<std::endl;
  	for (int j =0; j < temperaturelist.size(); j++){
  		std::cout << " Temperature " << temperaturelist[j] <<std::endl;
  		std::cout << " Region " << regionID[j] <<std::endl;
  		std::cout << " Direction " << heatdir[j] <<std::endl;
  			
  	}
	for (int k = 0; k < numberofregion; k++) {
			std::vector<int>* temp = new std::vector<int>;
			CriticalSectionAtomList.push_back(temp);
		}
      ...
      if (t_start <= 0.0 || t_stop <= 0.0)
        error->all(FLERR,
                   "Target temperature for fix nvtheat cannot be 0.0");
      //iarg += 4;
      iarg += narg-3;
    }   else error->all(FLERR,"Illegal fix nvtheat command");
  }
  ...
  if (dimension == 2 && (p_flag[2] || p_flag[3] || p_flag[4]))
    error->all(FLERR,"Invalid fix nvtheat command for a 2d simulation");
  ...
  if (pcouple == XYZ && (p_flag[0] == 0 || p_flag[1] == 0))
    error->all(FLERR,"Invalid fix nvtheat command pressure settings");
  ...
  if (p_flag[0] && domain->xperiodic == 0)
    error->all(FLERR,"Cannot use fix nvtheat on a non-periodic dimension");
  ...
  if (p_flag[3] && domain->zperiodic == 0)
    error->all(FLERR,
               "Cannot use fix nvtheat on a 2nd non-periodic dimension");
  ...
  if (scaleyz == 1 && domain->zperiodic == 0)
    error->all(FLERR,"Cannot use fix nvtheat "
  ...
  if (p_flag[3] && scaleyz == 1)
    error->all(FLERR,"Cannot use fix nvtheat with "
  ...
  if (!domain->triclinic && (p_flag[3] || p_flag[4] || p_flag[5]))
    error->all(FLERR,"Can not specify Pxy/Pxz/Pyz in "
               "fix nvtheat with non-triclinic box");

  if (pcouple == XYZ && dimension == 3 &&
      (p_start[0] != p_start[1] || p_start[0] != p_start[2] ||
       p_stop[0] != p_stop[1] || p_stop[0] != p_stop[2] ||
       p_period[0] != p_period[1] || p_period[0] != p_period[2]))
    error->all(FLERR,"Invalid fix nvtheat pressure settings");
  ...
  if ((tstat_flag && t_period <= 0.0) ||
    ...
    error->all(FLERR,"Fix nvtheat damping parameters must be > 0.0");
...
void FixNHheat::init()
{
  ...
  if (allremap == 0) {
    ...
    if (idilate == -1)
      error->all(FLERR,"Fix nvtheat dilate group ID does not exist");
...
void FixNHheat::setup(int /*vflag*/)
{
...
void FixNHheat::initial_integrate(int /*vflag*/)
{
  ...
  if (tstat_flag) {
	  if (!atom->normal_processing_flag == 0){
		  compute_temp_target();
		  nhc_temp_integrate();
  }
...
void FixNHheat::final_integrate()
{
  nve_v();
  //Added by manish/Shishir
   currentCount++;
   if (atom->normal_processing_flag == 0) {
	   //std::cout << "heating " <<std::endl;
 	  SelectAtomForHeating();
 	  HeatAtoms();
   }
   if (currentCount == atom->EquilibriumCounter) {
	   std::cout << "heating will start from next step " <<std::endl;
	   atom->normal_processing_flag = 0;
   }
   //**********************************
...
void FixNHheat::initial_integrate_respa(int /*vflag*/, int ilevel, int /*iloop*/)
{
...
void FixNHheat::final_integrate_respa(int ilevel, int /*iloop*/)
{
  // set timesteps by level
...
void FixNHheat::write_restart(FILE *fp)
{
...
int FixNHheat::size_restart_global()
{
  int nsize = 2;
...
int FixNHheat::pack_restart_data(double *list)
{
  int n = 0;
...
void FixNHheat::restart(char *buf)
{
  int n = 0;
...
int FixNHheat::modify_param(int narg, char **arg)
{
...
double FixNHheat::compute_scalar()
{
  int i;
  double volume;
  double energy;
...
double FixNHheat::compute_vector(int n)
{
  int ilen;
...
void FixNHheat::reset_target(double t_new)
{
...
void *FixNHheat::extract(const char *str, int &dim)
{
  dim=0;
...
void FixNHheat::nhc_temp_integrate()
{
  int ich;
  double expfac;
  double kecurrent = tdof * boltz * t_current;
 //std::cout << " AM I Here ? Confused " << std::endl;
...
void FixNHheat::nhc_press_integrate()
{
  int ich,i,pdof;
...
double FixNHheat::compute_strain_energy()
{
...
void FixNHheat::compute_deviatoric()
{
...
void FixNHheat::compute_temp_target()
{
...
void FixNHheat::compute_press_target()
{
  double delta = update->ntimestep - update->beginstep;
  if (delta != 0.0) delta /= update->endstep - update->beginstep;
...
void FixNHheat::SelectAtomForHeating() {
	double **x = atom->x;
	int *type = atom->type;
	int nlocal = atom->nlocal;
	int nmax = atom->nmax;
	...
	if(polarflag){
		for (int i = 0; i < nmax; i++) {
			int itype = type[i];
			if (itype == 1) {	
				//std::cout << " fx "<<f[i][0] << " fy "<<f[i][1] << " fz "<<f[i][2] << "\n";
				//std::cout << " fx11 "<<f11[i][0] << " fy11 "<<f11[i][1] << " fz11 "<<f11[i][2] << "\n";
				//std::cout << " fx12 "<<f12[i][0] << " fy12 "<<f12[i][1] << " fz12"<<f12[i][2] << "\n";    
				for (int k = 0; k < numberofregion; k++) {
					int iregion = regionID[k];
					//std::cout <<" region name " << iregion <<std::endl;
					int direction = heatdir[k];
					//std::cout <<" direction " << direction <<std::endl;
					double xscale = domain->regions[iregion]->xscale;
					double yscale = domain->regions[iregion]->yscale;
					double zscale = domain->regions[iregion]->zscale;
					//std::cout << " x "<<xscale*x[i][0] << " y "<<yscale*x[i][1] << " z"<<zscale*x[i][2] << "\n";
					//std::cout << " x "<<xscale*x[i][0] << " y "<<yscale*x[i][1] << " z"<<zscale*x[i][2] << "\n"
					if(domain->regions[iregion]->inside(xscale*x[i][0], yscale*x[i][1], zscale*x[i][2])){
						double Force11 = f11[i][direction];
						double Force12 = f12[i][direction];
						//std::cout <<" force 11  " << Force11 <<std::endl;
						//std::cout <<" force 12  " << Force12 <<std::endl;
						if (fabs(Force11) < fabs(Force12)) {
							CriticalSectionAtomList[k]->push_back(i);
							//std::cout <<" k " << k+1 << " atom id " << i <<std::endl;
						}
						//else 
						//std::cout << "Not getiing into the region " << std::endl;
					}
					//else 
					//std::cout << "Not getiing into the region " << std::endl;
				}
			}
			f11[i][0] =f11[i][1]=f11[i][2]=f12[i][0]=f12[i][1]=f12[i][2] = 0;
		}
	}
	else{
		for (int i = 0; i < nmax; ++i) {
			f11[i][0] = f[i][0]-f12[i][0] ;
			f11[i][1] = f[i][1]-f12[i][1] ;
			f11[i][2] = f[i][2]-f12[i][2] ;
		}
		for (int i = 0; i < nmax; i++) {
			int itype = type[i];
			if (itype == 3) {	
...
void FixNHheat::HeatAtoms() {
	...
	std::ofstream fout;
	fout.open("CriticalAtomList.txt");
  	/*std::cout << " number of region " << temperaturelist.size() <<std::endl;
  	for (int j =0; j < CriticalSectionAtomList.size(); j++){
  		std::cout << " Temperature " << temperaturelist[j] <<std::endl;
  		std::cout << " Region " << regionID[j] <<std::endl;
  		std::cout << " Direction " << heatdir[j] <<std::endl;
  			
  	}*/
	for (size_t r = 0; r < CriticalSectionAtomList.size(); r++) {
		fout << "**Region number " << r + 1 << "\n";
		double targettemp = temperaturelist[r];
		//std::cout << " region number "<< r+1 << " target temp   " << targettemp <<std::endl;
		for (size_t i = 0; i < CriticalSectionAtomList[r]->size(); i++) {
			int _CurrentAtom = CriticalSectionAtomList[r]->at(i);
			int _Currenttype = type[_CurrentAtom];
			double CurrentTemp =
					force->mvv2e
					* ((v[_CurrentAtom][0] * v[_CurrentAtom][0]
															 + v[_CurrentAtom][1] * v[_CurrentAtom][1]
																									+ v[_CurrentAtom][2] * v[_CurrentAtom][2])
							* mass[_Currenttype]) / (3 * force->boltz);
			double factor = sqrt(targettemp / CurrentTemp);
			//fout << " current atom _ in heat " << _CurrentAtom <<"\n";
			//std::cout << " m1  " << mass[1] <<std::endl;
			//std::cout << " factor  " << factor <<std::endl;
			//if ( temperaturelist[0] != 90) std::cout << " should be 90  "<<std::endl;
			//if ( temperaturelist[1] != 140) std::cout << " should be 140  "<<std::endl;
			
			fout << _CurrentAtom << "	" << x[_CurrentAtom][0] << "	"
					<< x[_CurrentAtom][1] << "	" << x[_CurrentAtom][2] << "	" << v[_CurrentAtom][0] << "	"
					<< v[_CurrentAtom][1] << "	" << v[_CurrentAtom][2] << "\n";
			v[_CurrentAtom][0] *= factor;
			v[_CurrentAtom][1] *= factor;
			v[_CurrentAtom][2] *= factor;
			fout << _CurrentAtom << "	" << x[_CurrentAtom][0] << "	"
							<< x[_CurrentAtom][1] << "	" << x[_CurrentAtom][2] << "	" << v[_CurrentAtom][0] << "	"
							<< v[_CurrentAtom][1] << "	" << v[_CurrentAtom][2] << "\n";

		}
		CriticalSectionAtomList[r]->clear();
		//std::cout << "sixzr of critical after" << CriticalSectionAtomList[r]->size() <<std::endl;
	}
	fout << "closing file";
	fout.close();
	
	//CriticalSectionAtomList.clear();
