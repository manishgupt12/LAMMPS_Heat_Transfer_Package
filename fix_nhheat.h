...
#ifndef LMP_FIX_NHHEAT_H
#define LMP_FIX_NHHEAT_H
...
class FixNHheat : public Fix {
 public:
	FixNHheat(class LAMMPS *, int, char **);
  virtual ~FixNHheat();
  int setmask();
  virtual void init();
 ...
 protected:
  ...
  void nh_omega_dot();
  int currentCount;
  int numberofregion; 				//Added by manish
  std::vector<int> regionID;
  std::vector<double> temperaturelist;
  std::vector<int> heatdir ; 
  std::vector<std::vector<int>*> CriticalSectionAtomList;
  bool polarflag = true;
  //double direction;
  //int iregion;
};
