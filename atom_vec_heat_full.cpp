...
AtomVecheatFull::AtomVecheatFull(LAMMPS *lmp) : AtomVec(lmp)
{
  molecular = 1;
...
void AtomVecheatFull::grow(int n)
{
  if (n == 0) grow_nmax();
  else nmax = n;
  atom->nmax = nmax;
  ...
  f11 = memory->grow(atom->f11,nmax*comm->nthreads,3,"atom:f11");
  f12 = memory->grow(atom->f12,nmax*comm->nthreads,3,"atom:f12");
...
void AtomVecheatFull::grow_reset()
{
  tag = atom->tag; type = atom->type;
  mask = atom->mask; image = atom->image;
  x = atom->x; v = atom->v; f = atom->f;f11 = atom->f11;f12 = atom->f12;
...
void AtomVecheatFull::copy(int i, int j, int delflag)
{
  int k;
...
int AtomVecheatFull::pack_comm(int n, int *list, double *buf,
...
int AtomVecheatFull::pack_comm_vel(int n, int *list, double *buf,
                               int pbc_flag, int *pbc)
{
  int i,j,m;
...
void AtomVecheatFull::unpack_comm(int n, int first, double *buf)
{
  int i,m,last;
...
void AtomVecheatFull::unpack_comm_vel(int n, int first, double *buf)
{
  int i,m,last;
...
int AtomVecheatFull::pack_reverse(int n, int first, double *buf)
{
  int i,m,last;
...
void AtomVecheatFull::unpack_reverse(int n, int *list, double *buf)
{
  int i,j,m;
...
int AtomVecheatFull::pack_border(int n, int *list, double *buf,
                             int pbc_flag, int *pbc)
{
  int i,j,m;
  double dx,dy,dz;
...
int AtomVecheatFull::pack_border_vel(int n, int *list, double *buf,
                                 int pbc_flag, int *pbc)
{
  int i,j,m;
  double dx,dy,dz,dvx,dvy,dvz;
...
int AtomVecheatFull::pack_border_hybrid(int n, int *list, double *buf)
{
  int i,j,m;
...
void AtomVecheatFull::unpack_border(int n, int first, double *buf)
{
  int i,m,last;
...
void AtomVecheatFull::unpack_border_vel(int n, int first, double *buf)
{
  int i,m,last;
...
int AtomVecheatFull::unpack_border_hybrid(int n, int first, double *buf)
{
  int i,m,last;
...
int AtomVecheatFull::pack_exchange(int i, double *buf)
{
  int k;
...
int AtomVecheatFull::unpack_exchange(double *buf)
{
  int k;
...
int AtomVecheatFull::size_restart()
{
  int i;
...
int AtomVecheatFull::pack_restart(int i, double *buf)
{
  int k;
...
int AtomVecheatFull::unpack_restart(double *buf)
{
  int k;
...
void AtomVecheatFull::create_atom(int itype, double *coord)
{
  int nlocal = atom->nlocal;
  if (nlocal == nmax) grow(0);
...
void AtomVecheatFull::data_atom(double *coord, imageint imagetmp, char **values)
{
  int nlocal = atom->nlocal;
  if (nlocal == nmax) grow(0);
...
int AtomVecheatFull::data_atom_hybrid(int nlocal, char **values)
{
...
void AtomVecheatFull::pack_data(double **buf)
{
  int nlocal = atom->nlocal;
...
int AtomVecheatFull::pack_data_hybrid(int i, double *buf)
{
...
void AtomVecheatFull::write_data(FILE *fp, int n, double **buf)
{
  for (int i = 0; i < n; i++)
...
int AtomVecheatFull::write_data_hybrid(FILE *fp, double *buf)
{
...
bigint AtomVecheatFull::memory_usage()
{
  bigint bytes = 0;
  ...
  if (atom->memcheck("f11")) bytes += memory->usage(f11,nmax*comm->nthreads,3);
  if (atom->memcheck("f12")) bytes += memory->usage(f12,nmax*comm->nthreads,3);
